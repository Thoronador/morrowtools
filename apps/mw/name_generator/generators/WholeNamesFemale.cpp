...
WholeNamesFemale::WholeNamesFemale()
: names({ })
{
}

std::vector<std::string> WholeNamesFemale::generate(const uint_least16_t n, const std::string& raceId)
{
  if (names.empty())
  {
    prepare(raceId);
...
void WholeNamesFemale::prepare(const std::string& raceId)
{
  ...
  while (iter != NPCs::get().end())
  {
    if (iter->second.RaceID == raceId)
    {
      ...
      // Name can only be used if gender matches.
      if (female)
