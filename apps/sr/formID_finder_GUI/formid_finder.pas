{
 -------------------------------------------------------------------------------
    ...
    Copyright (C) 2012, 2013, 2024  Dirk Stolle
...
begin
  ...
  // zero means we are in the child process
    ...
    // If we get to this point, an error occurred, because execv() does not
...
begin
  ...
  begin
    ...
    case return of
      ...
      errString := errString + 'Unknown error! Error code is '
                 + IntToStr(return) + '.';
    end; // case
...
begin
  ...
  m_StringData := '';
  {$IFDEF WINDOWS }
  m_PrevWndProc := Windows.WNDPROC(SetWindowLongPtr(Self.Handle, GWL_WNDPROC, PtrInt(@WndCallback)));
  {$ENDIF}
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('Search finished. Number of matching objects is '
                 + IntToStr(total) + '.');
      Exit;
    end // if last line
