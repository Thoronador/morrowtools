...
bool getSkryrimPathFromRegistry(std::string& pathData)
{
  ...
  #else
  HKEY resultingHKEY;
  LONG res = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Bethesda Softworks\\Skyrim", 0, KEY_READ, &resultingHKEY);
  ...
  if (res==ERROR_FILE_NOT_FOUND)
  {
    std::cout << "Error: Registry value for Skyrim's install path was not found!\n";
