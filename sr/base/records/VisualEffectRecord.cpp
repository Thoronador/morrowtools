...
#include "VisualEffectRecord.hpp"
#include <iostream>
#include <cstring>
...
bool VisualEffectRecord::saveToStream(std::ostream& output) const
{
...
bool VisualEffectRecord::loadFromStream(std::istream& in_File, const bool localized, const StringTable& table)
{
  uint32_t readSize = 0;
  ...
  if (subLength>511)
  {
    std::cerr <<"Error: sub record EDID of RFCT is longer than 511 characters!\n";
    return false;
  }
  //read EDID's stuff
  char buffer[512];
  ...
  if (!in_File.good())
  {
    std::cerr << "Error while reading subrecord EDID of RFCT!\n";
    return false;
  }
  ...
  if (subLength!=12)
  {
    std::cerr <<"Error: sub record DATA of RFCT has invalid length ("<<subLength
  ...
  if (!in_File.good())
  {
    std::cerr << "Error while reading subrecord DATA of RFCT!\n";
    return false;
  }
