...
#include "../../../../../apps/mw/name_generator/generators/SplitNamesMale.hpp"

TEST_CASE("MWTP::SplitNamesMale")
{
  ...
  SECTION("generate")
  {
    ...
    SplitNamesMale generator;
    REQUIRE( generator.generate(1, "SomeTestId").size() == 1 );
    REQUIRE( generator.generate(7, "SomeTestId").size() == 7 );
    const auto names = generator.generate(60, "SomeTestId");
    ...
    // Names "Jane" should not be present, neither with "Example" nor "Doe" as last name.
    REQUIRE( std::find(names.begin(), names.end(), "Jane Doe") == names.end() );
    REQUIRE( std::find(names.begin(), names.end(), "Jane Example") == names.end() );
    // Names "Barbara" should not be present, neither with "Example" nor "Doe" as last name.
    ...
    // Names "John" should be present with either "Example" or "Doe" as last name.
    REQUIRE( std::find(names.begin(), names.end(), "John Doe") != names.end() );
    REQUIRE( std::find(names.begin(), names.end(), "John Example") != names.end() );
    // Names "Bob" should be present with either "Example" or "Doe" as last name.
