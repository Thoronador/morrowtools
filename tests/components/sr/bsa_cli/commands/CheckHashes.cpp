...
TEST_CASE("bsa_cli::CheckHashes")
{
  ...
  {
    const auto arguments = std::string("a.out\0check-hashes\0foo_hashes.bsa\0fail\0"sv);
    ...
    REQUIRE( argv[1] == "check-hashes"s );
    REQUIRE( argv[2] == "foo_hashes.bsa"s );
    ...
    // create "BSA" file
    std::ofstream bsa("foo_hashes.bsa", std::ios::trunc | std::ios::out);
    bsa.close();

    REQUIRE( command.parseArguments(3, argv) == 0 );

    // cleanup: delete file
    REQUIRE( deleteFile("foo_hashes.bsa") );
  }

  SECTION("run: fail with empty file")
  {
    const auto arguments = std::string("a.out\0check-hashes\0foo_hashes_run.bsa\0"sv);
    ...
    REQUIRE( argv[0] == "a.out"s );
    REQUIRE( argv[1] == "check-hashes"s );
    REQUIRE( argv[2] == "foo_hashes_run.bsa"s );

    CheckHashes command;

    // create "BSA" file
    std::ofstream bsa("foo_hashes_run.bsa", std::ios::trunc | std::ios::out);
    bsa.close();
    ...
    // cleanup: delete file
    REQUIRE( deleteFile("foo_hashes_run.bsa") );
  }
