...
TEST_CASE("bsa_cli::ExtractAll")
{
  ...
  {
    ExtractAll command;
    REQUIRE( command.parseArguments(4, nullptr) != 0 );
  }

  SECTION("parseArguments")
  {
    const auto arguments = std::string("a.out\0extract-all\0foo_extract_all.bsa\0new_dir\0fail"sv);
    std::array<char*, 5> argArr = {
        ...
        const_cast<char*>(&arguments.c_str()[38]),
        const_cast<char*>(&arguments.c_str()[46])
    };
    char ** argv = argArr.data();

    REQUIRE( argv[0] == "a.out"s );
    REQUIRE( argv[1] == "extract-all"s );
    REQUIRE( argv[2] == "foo_extract_all.bsa"s );
    REQUIRE( argv[3] == "new_dir"s );
    REQUIRE( argv[4] == "fail"s );
    ...
    // create "BSA" file
    std::ofstream bsa("foo_extract_all.bsa", std::ios::trunc | std::ios::out);
    bsa.close();

    REQUIRE( command.parseArguments(4, argv) == 0 );

    // cleanup: delete file
    REQUIRE( deleteFile("foo_extract_all.bsa") );
  }

  SECTION("run: fail with empty file")
  {
    const auto arguments = std::string("a.out\0extract-all\0foo_extract_all.bsa\0new_dir\0"sv);
