...
#include "../../../../../apps/mw/name_generator/generators/SplitNamesFemale.hpp"

TEST_CASE("MWTP::SplitNamesFemale")
{
  ...
  SECTION("generate")
  {
    ...
    SplitNamesFemale generator;
    REQUIRE( generator.generate(2, "SomeTestId").size() == 2 );
    ...
    // Names "John" should not be present, neither with "Example" nor "Doe" as last name.
    REQUIRE( std::find(names.begin(), names.end(), "John Doe") == names.end() );
    REQUIRE( std::find(names.begin(), names.end(), "John Example") == names.end() );
    // Names "Bob" should not be present, neither with "Example" nor "Doe" as last name.
